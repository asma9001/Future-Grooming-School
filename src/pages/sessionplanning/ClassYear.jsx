...
export default function ClassYear() {
  ...
  const [filteredClassYears, setFilteredClassYears] = useState(classYears)
  ...
  const handleSubmit = (e) => {
    ...
    const newClassYear = {
      id: classYears.length > 0 ? Math.max(...classYears.map((cy) => cy.id)) + 1 : 1,
    ...
    // Add to class years data
    setClassYears([...classYears, newClassYear])
  ...
  const confirmStatusChange = () => {
    ...
    setClassYears(classYears.map((cy) => (cy.id === classYearId ? { ...cy, isActive: newStatus } : cy)))
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredClassYears.length > 0 ? (
                  filteredClassYears.map((classYear) => (
                    <tr key={classYear.id} className="hover:bg-gray-50 transition-colors">
