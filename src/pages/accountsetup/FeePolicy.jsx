...
const FeePolicy = () => {
  ...
  const [policies, setPolicies] = useState([
    {
      ...
      class: "PLAY",
      year: "2024",
      feeName: "PLAY Class Fee",
      fee: "2000",
      feeAfterDueDate: "2000",
      feesDetails: {
        tuitionFee: 1800,
        securityFee: 100,
        sportsFee: 100,
  ...
  const confirmStatusChange = () => {
    setPolicies(
      policies.map((policy) =>
        policy.id === confirmAction.id ? { ...policy, status: confirmAction.newStatus } : policy,
      ),
    )
  ...
  const formatFeesDetails = (feesDetails) => {
    ...
    const total = feesDetails.tuitionFee + feesDetails.securityFee + feesDetails.sportsFee
                ...
                {filteredPolicies.map((policy) => (
                  <tr key={policy.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm">
                      {policy.status === "Yes" ? (
                        <button
                          onClick={() => handleStatusChange(policy.id, "No")}
                        ...
                        <button
                          onClick={() => handleStatusChange(policy.id, "Yes")}
          ...
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md mx-4 transform transition-all">
            ...
            <div className="bg-gray-50 rounded-lg p-3 mb-6">
              <div className="text-sm text-gray-600">
                <strong>Policy:</strong> {policies.find((p) => p.id === confirmAction?.id)?.feeName}
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className={`flex-1 px-4 py-2 text-white rounded-lg transition-colors ${
                  confirmAction?.type === "activate" ? "bg-green-600 hover:bg-green-700" : "bg-red-600 hover:bg-red-700"
                }`}
              >
