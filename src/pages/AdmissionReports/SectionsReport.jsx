    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-blue-700 text-white p-4 rounded-t-lg flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">Section Wise Students Report</h1>
        </div>

        {/* Main Content */}
        <div className="bg-white rounded-b-lg shadow-lg">
          {/* Replace the Search and Filter Section with the layout from AdmissionYear */}
          <div className="p-6 border-b border-gray-200">
            ...
            {searchTerm && (
              <div className="mt-2 text-sm text-gray-600">
                Showing {filteredData.length} of {sectionsData.length} results for "{searchTerm}"
              </div>
            )}
          </div>
                ...
                {filteredData.length > 0 ? (
                  filteredData.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50 transition-colors duration-200">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{item.classYear}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-center">{item.section}</td>
