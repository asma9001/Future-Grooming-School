...
const Gender = () => {
  ...
  const filteredData = genderData.filter((item) => {
    ...
    switch (filterColumn) {
      ...
      default:
        return item.gender.toLowerCase().includes(searchLower) || item.students.toString().includes(searchTerm)
    }
  })
          ...
          <div className="p-6 border-b border-gray-200">
            ...
            {searchTerm && (
              <div className="mt-2 text-sm text-gray-600">
                Showing {filteredData.length} of {genderData.length} results for "{searchTerm}"
              </div>
            )}
          </div>
                ...
                {filteredData.length > 0 ? (
                  filteredData.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50 transition-colors duration-200">
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-center">{item.gender}</td>
